# 🌱 Equipo 08 | Proyecto Integrador

<p align="center">
  <img src="https://img.shields.io/badge/Universidad-Peruana%20Cayetano%20Heredia-1B5E20?style=for-the-badge" alt="Universidad"/>
  <img src="https://img.shields.io/badge/Curso-Nombre%20del%20curso-2E7D32?style=for-the-badge" alt="Curso"/>
  <img src="https://img.shields.io/badge/Ciclo-202X--1-43A047?style=for-the-badge" alt="Ciclo"/>
</p>

<p align="center">
  <strong>🎓 Carrera de Ingeniería Ambiental / Informática / Industrial</strong>
</p>

<p align="center">
  <em>Innovación · Tecnología · Sostenibilidad · Impacto Social</em>
</p>

---

## 🌍 Sobre Nosotros

Somos el **Equipo 0X** del curso **Nombre del curso 202X-1**, conformado por estudiantes de la **Universidad Peruana Cayetano Heredia**.

Nuestro objetivo es aplicar la **metodología de diseño** para identificar problemas, desarrollar propuestas y generar **soluciones innovadoras** con impacto:

> 🌱 **Ambiental** · 💡 **Tecnológico** · 🤝 **Social**

Buscamos integrar nuestros conocimientos y habilidades para desarrollar propuestas que respondan a desafíos reales y contribuyan a un futuro más sostenible.

---

## 🎯 Objetivos de Desarrollo Sostenible

Nuestro trabajo se encuentra alineado con los siguientes **Objetivos de Desarrollo Sostenible (ODS)**:

|       ODS      | Objetivo                                | Enfoque                              |
| :------------: | --------------------------------------- | ------------------------------------ |
|  🩺 **ODS 3**  | Salud y Bienestar                       | Promover una mejor calidad de vida   |
|  💧 **ODS 6**  | Agua Limpia y Saneamiento               | Gestión sostenible del agua          |
|  🏭 **ODS 9**  | Industria, Innovación e Infraestructura | Innovación y desarrollo tecnológico  |
| 🏙️ **ODS 11** | Ciudades y Comunidades Sostenibles      | Construcción de entornos sostenibles |
|  🌎 **ODS 13** | Acción por el Clima                     | Respuesta frente al cambio climático |

---

## 📸 Nuestro Equipo

<p align="center">
  <img 
    src="https://github.com/user-attachments/assets/3829718f-51c7-4d00-b262-ef37f178f059"
    alt="Fotografía del equipo 0X"
    width="850"
  />
</p>

<p align="center">
  <em>Figura 1. Fotografía del Equipo 0X</em>
</p>

---

## 👥 Integrantes

<table align="center">
  <tr>
    <th>Foto</th>
    <th>Integrante</th>
    <th>Rol</th>
    <th>Intereses</th>
  </tr>

  <tr>
    <td align="center">
      <img src="/Recursos/Imágenes/integrante1.png" width="90"/>
    </td>
    <td><strong>Nombre 1</strong></td>
    <td>👑 Líder del equipo</td>
    <td>Innovación social, sostenibilidad</td>
  </tr>

  <tr>
    <td align="center">
      <img src="/Recursos/Imágenes/integrante2.png" width="90"/>
    </td>
    <td><strong>Nombre 2</strong></td>
    <td>🔎 Responsable de investigación</td>
    <td>Gestión ambiental, desarrollo comunitario</td>
  </tr>

  <tr>
    <td align="center">
      <img src="/Recursos/Imágenes/integrante1.png" width="90"/>
    </td>
    <td><strong>Nombre 3</strong></td>
    <td>🎨 Diseñador/a</td>
    <td>Diseño de prototipos, creatividad aplicada</td>
  </tr>

  <tr>
    <td align="center">
      <img src="/Recursos/Imágenes/integrante2.png" width="90"/>
    </td>
    <td><strong>Nombre 4</strong></td>
    <td>📝 Documentación</td>
    <td>Comunicación científica, redacción técnica</td>
  </tr>

  <tr>
    <td align="center">
      <img src="/Recursos/Imágenes/integrante1.png" width="90"/>
    </td>
    <td><strong>Nombre 5</strong></td>
    <td>💻 Programador/a - Modelador/a</td>
    <td>Programación, análisis de datos, simulación</td>
  </tr>
</table>

---

## 🚀 Nuestro Enfoque

Durante el desarrollo del curso, trabajaremos en las diferentes etapas de la **metodología de diseño**, buscando transformar problemas en oportunidades de innovación.

```text
🔎 Investigación
      ↓
💭 Ideación
      ↓
🛠️ Diseño y Prototipado
      ↓
🧪 Evaluación
      ↓
🌱 Solución con Impacto
```

Nuestro propósito es combinar **conocimiento, creatividad y tecnología** para construir propuestas que puedan generar un impacto positivo.

---

## 📌 Resumen

Este repositorio representa el trabajo del **Equipo 0X** durante el curso **Nombre del curso 202X-1**.

Aquí documentaremos nuestro proceso, aprendizajes, propuestas y resultados, manteniendo como ejes principales:

<p align="center">

|   🌱 Sostenibilidad  |     💡 Innovación    |      🤝 Impacto Social     |
| :------------------: | :------------------: | :------------------------: |
| Cuidado del ambiente | Soluciones creativas | Beneficio para la sociedad |

</p>

---

## 🎓 Universidad Peruana Cayetano Heredia

<p align="center">
  <strong>Equipo 0X · Nombre del curso · 202X-1</strong>
  <br>
  <em>Diseñando soluciones para un futuro sostenible.</em>
</p>

<p align="center">
  🌱 💡 🌎 🤝
</p>
